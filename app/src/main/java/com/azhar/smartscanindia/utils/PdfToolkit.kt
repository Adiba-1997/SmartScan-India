    ...
    fun compressPdf(context: Context, input: File): File {
        ...
        try {
            for (i in 0 until renderer.pageCount) {
                ...
                val targetW = (page.width * 0.7f).toInt().coerceAtLeast(800)
                val targetH = (page.height * 0.7f).toInt().coerceAtLeast(1100)
                val bitmap = Bitmap.createBitmap(targetW, targetH, Bitmap.Config.ARGB_8888)
                page.render(bitmap, null, null, PdfRenderer.Page.RENDER_MODE_FOR_PRINT)
                val imageFile = FileUtils.newImageFile(context, "COMP_PAGE_${i + 1}")
                FileOutputStream(imageFile).use { bitmap.compress(Bitmap.CompressFormat.JPEG, 65, it) }
        ...
        return imageToPdf(context, pagesAsImages, "compressed_${input.name}", jpegQuality = 0.6f)
    }
